                ...
                foreach (var directory in commandSetDirectories)
                {
                    ...
                    if (File.Exists(commandJsonPath))
                    {
                        ...
                        if (commandSetData != null)
                        {
                            ...
                            {
                                // Create a command configuration, but determine supported versions by checking folders
                                ...
                                foreach (var version in versionDirectories)
                                {
                                    ...
                                    {
                                        // If a relative path is specified, search in version subfolders
                                        ...
                                        if (File.Exists(versionDllPath))
                                        {
                                            ...
                                            if (dllBasePath == null)
                                            {
                                                // Extract relative path for template creation
                                    ...
                                    {
                                        // If no path is specified, search for any DLL in version subfolders
        ...
        private void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                foreach (var commandSet in commandSets)
                {
                    ...
                    {
                        // Only add enabled commands to registry
                        if (command.Enabled)
                        {
                            CommandConfig newConfig;
                            // Check if command already exists in previous registry
                            if (existingCommandsDict.ContainsKey(command.CommandName))
                            {
                                // If exists, preserve original information, only update enabled status and path template
